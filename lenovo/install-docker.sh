...
echo "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/ubuntu $DISTRO_RELEASE stable" | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null
...
# Create the docker group (ignore if already exists).
